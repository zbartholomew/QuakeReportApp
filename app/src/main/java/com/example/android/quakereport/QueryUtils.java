...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            // Getting JSON feature array node
            JSONArray featuresArray = jsonObj.getJSONArray("features");

            for (int i =0; i < featuresArray.length(); i++) {
                // create JSON object to retrieve each earthquake data in features node
                JSONObject currentEarthquake = featuresArray.getJSONObject(i);
                // the properties object contains all the information we want
                JSONObject properties = currentEarthquake.getJSONObject("properties");

                // get all necessary data from property node
                ...
                String dateDisplay = dateFormat.format(dateObj);

                // create an object for each earthquake and store it in the earthquakes object
